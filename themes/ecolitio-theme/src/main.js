...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const changeImagePatinete = () => {
    ...
    const ubicaciones = document.querySelectorAll(
      'input[name="ubicacion-de-bateria"]'
    );
  ...
  const updatePriceVariableProduct = () => {
    ...
    const siblingDescription = document.querySelector(
    ...
    const handlePriceUpdate = () => {
      ...
      if (varDescription && varDescription.innerHTML.trim()) {
        ...
        if (
          existingVarDesc &&
          existingVarDesc.classList.contains(
            "woocommerce-variation-description"
          )
        ) {
          ...
          siblingDescription.parentNode.insertBefore(
            newDescElement,
            siblingDescription
          );
        }
