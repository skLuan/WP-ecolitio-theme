						...
						<div class="label-container grid grid-cols-2 gap-2 grid-rows-2 justify-evenly">
							...
							foreach ($connector_values as $option) :
								...
								<label for="input-connector-<?= esc_attr($option); ?>" class="">
									...
									<?php if($option !== 'OTROS') : ?>
									<figure class="cursor-pointer border-white-eco rounded-lg overflow-hidden" style="border-color: var(--battery-text-color); border-width: 1px;" onmouseover="if(!this.parentElement.querySelector('input').checked) { this.style.borderColor='var(--battery-color)'; }" onmouseout="if(!this.parentElement.querySelector('input').checked) { this.style.borderColor='var(--battery-color)'; } else { this.style.borderColor='var(--battery-color)'; }">
										<picture>
